...
class AlpacaMHJit(jit.ScriptModule):
    """
    Multi-head implementation of ALPaCA, a meta-learning algorithm
    for online bayesian regression for an imitation setting.
    """
    ...
    def __init__(self, config={}, cuda=0):
        ...
        if self.config['model.glorot_init']:
            ...
            self.Q = nn.Parameter(torch.randn(1, self.u_dim, 1, self.phi_dim) * (np.sqrt(2/(self.u_dim + self.phi_dim))))

            # L matrix used for Cholesky decomposition, i.e. A = LL^T. Also with Glorot initialization
            self.L_asym = nn.Parameter(torch.randn(1, self.u_dim, self.phi_dim, self.phi_dim) * (np.sqrt(2/(self.phi_dim + self.phi_dim))))

            # Scaling matrix. Must be a diagonal matrix with all positive entries
            self.L_scale = nn.Parameter(torch.eye(self.phi_dim) * self.config['model.init.L_scale'])
        else:
            # init from James' repo
            print('diff init')
            self.Q = nn.Parameter(torch.randn(1, self.u_dim, 1, self.phi_dim)*4/(np.sqrt(self.phi_dim)+ np.sqrt(self.u_dim)))
            self.L_asym = nn.Parameter(torch.randn(1, self.u_dim, self.phi_dim, self.phi_dim)/self.phi_dim**2)
            self.L_scale = nn.Parameter(torch.linspace(-5,0, self.phi_dim).repeat(self.u_dim,1))
        ...
        if self.config['model.simp_sig_ep']:
            self.logSigEps = nn.Parameter(torch.ones(self.u_dim)*self.config['model.sigep_scale'], requires_grad=self.config['training.learnable_noise'])
        else:
    ...
    @property
    def SigEp(self):
        return torch.exp(self.logSigEps).cuda()
    ...
    @jit.script_method
    def recursive_update(self, phi, u, z, stats: Tuple[torch.Tensor, torch.Tensor]):
        """
        Args:
        ...
        """
        stats = torch.jit.annotate(List[Tensor], stats)
        Linv, Q = stats
    ...
    @jit.script_method
    def log_predictive_prob(self, phi, u, z, stats: Tuple[torch.Tensor, torch.Tensor]):
        """
            Args:
                phi: encoding tensor (batch_dim, phi_dim)
                u: action tensor (batch_dim, u_dim)
        ...
        pred_cov_inv = torch.ones(pred_cov_diag.shape).cuda() / pred_cov_diag
...
class AlpacaMocaJit(jit.ScriptModule):
    """
    Multi-head implementation of ALPaCA, a meta-learning algorithm
    for online bayesian regression for use with Moca. The number of skills
    here corresponds to the number of potential run-lengths
    """
    def __init__(self, config={}, cuda=0):
        super().__init__()
    ...
    @jit.script_method
    def recursive_update(self, phi, u, stats: Tuple[torch.Tensor, torch.Tensor]):
        """
        Args:
            phi: encoding tensor (batch_dim, phi_dim)
            u: action tensor (batch_dim, u_dim)
    ...
    @jit.script_method
    def log_predictive_prob(self, phi, u, stats: Tuple[torch.Tensor, torch.Tensor]):
        """
            Args:
                phi: encoding tensor (batch_dim, phi_dim)
                u: action tensor (batch_dim, u_dim)
